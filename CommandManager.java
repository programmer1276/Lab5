...
public class CommandManager {
    ...
    public static void executeCommand(String command) throws InvalidCommandException {
        if (command != null) {
            String[] commandAndArgs = command.split(" ", 2);
            try {
                if (!(commands.containsKey(commandAndArgs[0]))) {
                    throw new InvalidCommandException("Программа не поддерживает данную команду, попробуйте еще раз");
                }
    ...
    public static void history() {
        MyConsole.println("Ваша история команд: \n----------------------------------");
        if (headIndex == -1) {
            MyConsole.println("История команд пуста");
        } else {
            int i = headIndex;
            int count = 1;
            do {
                MyConsole.println(count + ": " + commandHistory[i]);
                i = (i + 1) % HISTORY_SIZE;
                count++;
            } while (i != tailIndex + 1);
        }
